...
public class PlaceService {
    ...
    public List<Place> findInTwoKm(double latitude, double longitude) {
        List<Place> places = placeRepository.listAll();

        List<Place> placesInArea = new ArrayList<>();

        for (Place place : places) {
            Double placeLatitude = place.getLatitude();
            Double placeLongitude = place.getLongitude();

            if (calcolaDistanza(latitude, longitude, placeLatitude, placeLongitude) <= 2) {
                placesInArea.add(place);
            }
        }
